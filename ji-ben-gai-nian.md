...
&gt;提示:每个分片其实是一个Lucene索引，单个Lucene索引有最大的文档数限制，例如\(\[LUCENE-5843\]\(https://issues.apache.org/jira/browse/LUCENE-5843\)\)，阈值是2,147,483,519 \(= Integer.MAX\_VALUE - 128\) 个文档。你可以通过API \[\`\_cat/shards\`\]\(https://www.elastic.co/guide/en/elasticsearch/reference/2.2/cat-shards.html "cat shards"\)去监控分片的大小。
